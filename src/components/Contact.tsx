...
export function Contact() {
  return (
    <section className="py-16 md:py-24 px-4 pb-12 w-full max-w-6xl mx-auto border-t border-neutral-800/50 mt-12">
      <div className="grid lg:grid-cols-2 gap-8 md:gap-12">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="bg-neutral-900 border border-neutral-800 p-6 sm:p-8 md:p-12 rounded-2xl md:rounded-3xl relative overflow-hidden group"
        >
          <div className="absolute top-0 right-0 w-32 h-32 bg-primary/20 blur-[50px] rounded-full pointer-events-none" />
          <h3 className="text-xl sm:text-2xl font-bold text-white mb-4">Para Clientes ou Projetos</h3>
          <p className="text-sm sm:text-base text-neutral-400 mb-8 max-w-md">
        ...
        <motion.div
          ...
          className="bg-transparent border border-neutral-800 p-6 sm:p-8 md:p-12 rounded-2xl md:rounded-3xl relative overflow-hidden group hover:bg-neutral-900/40 transition-colors"
        >
          <div className="absolute top-0 right-0 w-32 h-32 bg-accent/20 blur-[50px] rounded-full pointer-events-none" />
          <h3 className="text-xl sm:text-2xl font-bold text-white mb-4">Para Recrutadores</h3>
          <p className="text-sm sm:text-base text-neutral-400 mb-8 max-w-md">
      ...
      <div className="mt-16 md:mt-24 text-center border-t border-neutral-800/50 pt-8 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-4">
        <p className="text-neutral-500 font-medium text-sm sm:text-base">© {new Date().getFullYear()} Lucas Gabriel. Todos os direitos reservados.</p>
